...
def load_data_img(args):
    ...
    captions = json.load(open(args.caption_file))["captions"]
    name_maps = json.load(open('data/scienceqa/name_map.json'))
    ...
    if args.img_type == "resnet":
        ...
        image_features = np.load('data/scienceqa/detr.npy')
    ...
    def __init__(
        ...
        for qid in self.data:
            if test_le_data is not None:
                ...
                curr_le_data = None
            data,prompt, target = build_train_pair(problems, qid, args, curr_le_data)
            if data is None:
                continue
