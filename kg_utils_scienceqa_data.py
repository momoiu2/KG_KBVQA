...
def load_okvqa_data_img(args):
    ...
    train_problems = json.load(open(os.path.join(args.data_root, 'scienceqa/train_data.json')))  # 问题
    test_problems = json.load(open(os.path.join(args.data_root, 'scienceqa/test_data.json')))
    val_problems = json.load(open(os.path.join(args.data_root, 'scienceqa/val_data.json')))
    ...
    captions = json.load(open('data/scienceqa/instruct_captions.json'))["captions"]



    train_name_maps = json.load(open('data/scienceqa/name_map.json'))
    test_name_maps = json.load(open('data/scienceqa/name_map.json'))
    val_name_maps = json.load(open('data/scienceqa/name_map.json'))
    start = args.start
    end = args.end
    ...
    for train_qid in train_qids:
        train_problems[train_qid]["caption"] = captions.get(train_qid,"")
    for test_qid in test_qids:
        test_problems[test_qid]["caption"] = captions.get(test_qid,"")
    print(f"number of train problems: {len(train_qids)}\n")
    print(f"number of val problems: {len(val_qids)}\n")
    print(f"number of test problems: {len(test_qids)}\n")
    if args.img_type == "resnet":
        ...
        train_image_features = np.load('data/scienceqa/detr.npy')
        val_image_features = np.load('data/scienceqa/detr.npy')
        test_image_features = np.load('data/scienceqa/detr.npy')
    else:
    ...
    def __init__(
        ...
        for qid in qids:
            ...
            if test_le_data is not None:
                ...
                curr_le_data = None
            data, prompt, target = build_train_pair(problems, qid, args,
                                                    curr_le_data)
            # print(prompt)
            # print(target)
            if data is None:
