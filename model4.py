...
# from bert_model import BertLMHeadModel
...
class T5ForMultimodalGeneration(T5ForConditionalGeneration):
    ...
    def forward(
        ...
        print(image_att)
        # image_att = image_att.to(device)
        x = torch.cat([hidden_states, image_att], dim=-1)#(1,512,1536)
        # x = image_att
        ...
        if self.add_residual:
            out = out + x
        # hidden_states = gau(out)
        hidden_states = out
        hidden_states = self.gate_dense(hidden_states)
        # hidden_states = hidden_states
        # gate = self.sigmoid(self.gate_dense(merge))#(1,512,768)
        # hidden_states = (1 - gate) * hidden_states + gate * image_att #(1,512,768) 融合
